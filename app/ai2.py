...
def yolo_forward(net, LABELS, image, confidence_level, save_image=False):
    ...
    # boxes
    idxs = cv2.dnn.NMSBoxes(boxes, confidences, confidence_level,
                            confidence_level)
    ...
    if save_image:
        yolo_save_img(image, class_ids, nms_boxes, labels, confidences, colors,
                      'python_predictions.jpg')

    return nms_class_ids, labels, nms_boxes, nms_confidences


def yolo_save_img(image, class_ids, boxes, labels, confidences, colors,
                  file_path):
    """
...
def yolo_pred(image_path, names_path, cfg_path, weight_path):
    ...
    (class_ids, labels, boxes,
     confidences) = yolo_forward(net, labels, image, confidence_level=0.5)

    yolo_show_img(image, class_ids, boxes, labels, confidences, colors)


def yolo_pred_list(image_folder_path,
                   names_file,
                   cfg_file,
                   weight_file,
